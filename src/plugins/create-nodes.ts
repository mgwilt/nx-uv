...
function createProjectNode(
  ...
  const projectName =
    ...
    (projectRoot === '.' ? 'python-workspace' : sanitizeName(posix.basename(projectRoot)));
...
function buildTargets(
  ...
  const baseTargets: Record<string, { executor: string; options: Record<string, unknown> }> = {
  ...
  if (includeGlobalTargets && projectRoot === '.' && pyproject.includes('[tool.uv.workspace]')) {
